...
## 步驟 1：找出待處理票單
呼叫 \`list_pending_tickets({ projectGid, sectionFilter?, projectName: <這個 Asana 專案的「全名稱」，步驟 0 拿到的> })\` **一定要帶 \`projectName\`**，取得這個 Asana 專案裡尚未完成、且尚未驗證通過（PASS）的票單清單。一張一張處理，不需要平行處理。帶了 \`projectName\` 之後，這次算出來的「待確認規格草稿／待確認／卡住需要介入／Asana 內容已變更待重新確認／需要你手動處理的事項／Git 尚未 commit 的變更」六類項目會自動整份寫進 \`<projectDir>/.asana-pipeline/<projectName>/PENDING_HUMAN_ACTIONS.html\`——這是持久化檔案，不是只在這次聊天回覆裡講一遍就消失，換 session、關掉對話都還在，步驟 3 的彙整報告不用再自己重複整理一次，直接告訴使用者這份檔案存在、位置在哪即可。這是一份可以互動的 HTML（不是純文字），使用者在瀏覽器打開就能勾選/確認事項；第一次告訴使用者這份檔案時，順便提醒他要在 \`dev-pipeline-mcp\` 目錄下跑一次 \`npm run start:http\` 啟動本機 HTTP bridge，互動按鈕才會生效。
...
2. **確認 SA/SD 規格（強制，不可跳過；但每個 Asana 專案通常只需要問一次）**：
   ...
   **硬性規定：SA 管前端、SD 管後端，兩份規格不能越界互相支援**——這是這條 pipeline 過去實際發生過的問題：分析師/工程師拿票單描述裡「聽起來合理」但 SD 沒寫的細節（甚至是套用 SD 另一個章節、原本只針對別的功能寫的規則）去幫後端加驗證邏輯，使用者看 code review 才發現規格書明明只有短短兩行，後端卻改了一大段。適用範圍：
   - **改後端程式碼（Controller/Service/Repository/Entity 等）時，只能依照 SD（程式規格）裡明確寫出來的欄位、業務邏輯、檢核規則去實作**——SD 沒寫的部分，就算票單描述、SA、或程式碼裡其他相似功能（例如同一支 Service 裡別的方法、匯入流程的驗證規則）看起來合理、可以直接套用，也不能自己延伸過去當作這張票要做的後端需求。
   - **改前端程式碼（畫面元件/表單驗證/顯示邏輯）時，只能依照 SA（需求規格書）裡的畫面、欄位、互動規則去實作**——不要把 SD 章節裡的後端邏輯細節（例如資料庫欄位對應、後端專屬的例外訊息）直接套用成前端的驗證規則或顯示文字。
   - **SD／SA 都沒有明確涵蓋、但票單描述或留言串有提到的細節，一律停下來問使用者這個落差要不要處理、算不算這張票的範圍**——不能自己判斷「這樣做比較保險/比較完整」就先斬後奏加進去；問清楚之後才能動手，也要把問答記錄進 \`01-analysis.md\`/\`02-implementation.md\`。
   - 這條規則不影響「同一支規格文件裡，後端規格本來就會提到少量前端顯示邏輯（例如某欄位唯讀/隱藏條件）」這種正常情況——重點是不要把「規格沒寫、只是分析師/工程師自己覺得合理」的部分，包裝成規格要求動手實作。
...
以下三個附錄是參考/選用材料，不是步驟 0～3 主流程的一部分，用不到的話可以跳過：

## 附錄 A：子任務派工建議（僅在你有能力派生子任務時適用）

**這一段只是建議，不是每個呼叫這個 MCP 的 AI 都適用，看你自己的能力決定要不要採用。**

如果你自己具備「派生子任務/子代理人去執行一段工作、並等待它回報結果」的能力（例如 Claude Code 的 Agent 工具，或其他等效機制），建議把步驟 2 之 4／4.5／5／6／6.5（分析師、規格撰寫者、工程師、驗證師、測試工程師這幾個角色的實際工作）改成派一個子任務去扮演該角色執行，你自己只做調度（確認上下文、事後驗證階段真的推進），不要自己套進角色直接分析、改程式碼、或下驗證結論。這樣可以讓你（調度者）維持在乾淨的判斷力狀態，不會被角色工作過程中的細節雜訊污染上下文，每個角色階段也更容易獨立重跑。

**如果你不具備這種能力**（例如一次性、單一上下文執行、沒有子任務機制的 AI host），完全不用勉強——就依照原本方式，呼叫 \`get_role_prompt\` 之後自己親自扮演該角色執行即可，步驟 2 之 4／4.5／5／6／6.5 的說明本來就是寫給「自己執行」用的。
...
- **工具使用提醒**：MCP 工具（\`mcp__dev-pipeline-mcp__*\`）如果子任務那邊還沒載入，要先載入；全程只透過這些工具讀寫票單追蹤紀錄跟程式碼，不要用主機環境原生的檔案/shell 工具去碰 \`projectDir\` 底下的程式碼——這樣才吃得到這個 MCP 本身的路徑沙盒跟 git push 封鎖。
...
## 附錄 B：換 session／換 AI 接手時，怎麼低成本接上進度
