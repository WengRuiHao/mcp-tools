...
**寫程式碼註解要節制，不要寫得太詳細**：只在真的需要說明「為什麼」時才寫（非顯而易見的限制、暫時性的權宜做法），不要把商業邏輯細節、內部規則、SQL/連線字串、帳密、API 金鑰、內部主機名稱或路徑這類資訊整段寫進註解——這些內容一旦寫進註解，程式碼被複製、分享、或之後這個 repo 轉為對外可見時就等於一併外洩，是資安風險，不是文件品質問題。程式碼本身看得懂在做什麼的地方，不需要額外註解重複說明。
