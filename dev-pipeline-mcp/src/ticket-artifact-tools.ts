...
export function registerTicketArtifactTools(server: McpServer): void {
  server.tool(
    ...
    "把內容寫入某張票單的追蹤目錄底下的一個檔案（例如 01-analysis.md / 02-implementation.md / 03-verification.md / 04-test.md）。" +
      ...
      "**filename 是 02-implementation.md／03-verification.md／04-test.md 時，manualActions 也是必填**（陣列，可以是空陣列）：這次有沒有任何事項是使用者必須自己手動處理的（例如產出的 SQL 只能交由使用者到 Database 工具執行、後台程式代號/選單/I18N 需自行設定；04-test.md 的話，還包含這次測試裡 AI 沒有精確依據判定、只能列出來提醒使用者親自確認的項目——needs_manual_check 分類的測試項目）？有就列成一條條簡短字串；真的沒有就帶空陣列 []，不能省略——這些項目會被整理進持久化的 PENDING_HUMAN_ACTIONS.md，不能只寫在全文內容裡指望使用者自己重讀全文才發現。" +
      ...
      "**寫入完成後會自動局部重寫這張票所屬 Asana 專案的 `PENDING_HUMAN_ACTIONS.md`**（純本機運算，不用另外呼叫 `list_pending_tickets`）。",
    {
      ...
      manualActions: z
        ...
        .optional()
        .describe(
          "filename 是 02-implementation.md／03-verification.md／04-test.md 時必填（陣列）。列出這次需要使用者手動處理的事項（例如「已產出 SQL，見內文，需自行到 Database 工具執行」；04-test.md 則是 needs_manual_check 分類的測試項目）；" +
  ...
  server.tool(
    ...
    "把 01-analysis.md／02-implementation.md／03-verification.md／04-test.md 其中一份檔案，現在磁碟上的實際內容重新雜湊、寫回 status.json 對應的 sync.*_hash 欄位。" +
      "**用在：這份檔案剛才是被直接用一般編輯工具（不是透過 write_ticket_artifact）手動改過的**——例如使用者跟另一個沒有走完整分析師/工程師/驗證師/測試工程師流程的 AI 直接對話改了內容。" +
      ...
      "**manualActions 也是選填（陣列）**：用在 `manualActions` 這個必填機制上線之前就已經寫過的舊 02/03/04 檔案——那些檔案裡可能藏著『SQL 待手動執行』『多國語系 I18N 需自行匯入』這類事項，但當時沒有結構化欄位可以宣告，全部散落在自由文字裡，`PENDING_HUMAN_ACTIONS.md` 抓不到。帶這個參數可以回填，讓舊票也正確出現在報告裡；只有 filename 是 02-implementation.md／03-verification.md／04-test.md 時才有意義（01 不支援）。",
    {
      taskGid: z.string().describe("Asana 任務 gid"),
      filename: z
