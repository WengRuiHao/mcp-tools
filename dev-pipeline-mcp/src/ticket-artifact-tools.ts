...
export function registerTicketArtifactTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "把 01-analysis.md／02-implementation.md／03-verification.md／04-test.md 其中一份檔案，現在磁碟上的實際內容重新雜湊、寫回 status.json 對應的 sync.*_hash 欄位。" +
      ...
      "**manualActions 也是選填（陣列）**：用在 `manualActions` 這個必填機制上線之前就已經寫過的舊 02/03/04 檔案——那些檔案裡可能藏著『SQL 待手動執行』『多國語系 I18N 需自行匯入』這類事項，但當時沒有結構化欄位可以宣告，全部散落在自由文字裡，`PENDING_HUMAN_ACTIONS.html` 抓不到。帶這個參數可以回填，讓舊票也正確出現在報告裡；只有 filename 是 02-implementation.md／03-verification.md／04-test.md 時才有意義（01 不支援）。",
    {
      taskGid: z.string().describe("Asana 任務 gid"),
      filename: z
        .enum(["01-analysis.md", "02-implementation.md", "03-verification.md", "04-test.md"])
