...
export function registerIssueTools(server: McpServer): void {
  server.tool(
    ...
    async ({ projectId, connectionId, state, search, labels, perPage, page }) =>
      toolResult(
        await gitlabListIssues(connectionId ?? undefined, projectId, {
  ...
  server.tool(
    ...
    { projectId: projectIdParam, connectionId: connectionIdParam, issueIid: issueIidParam },
    async ({ projectId, connectionId, issueIid }) => toolResult(await gitlabGetIssue(connectionId ?? undefined, projectId, issueIid))
  );
}
