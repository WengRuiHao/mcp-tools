...
export function registerFileTools(server: McpServer): void {
  server.tool(
    ...
    async ({ projectId, connectionId, ref, path, recursive }) =>
      toolResult(await gitlabGetRepositoryTree(connectionId ?? undefined, projectId, ref ?? undefined, path ?? undefined, recursive ?? undefined))
  );
  ...
  server.tool(
    ...
    async ({ projectId, connectionId, filePath, ref }) =>
      toolResult(decodeFileContent(await gitlabGetFileContents(connectionId ?? undefined, projectId, filePath, ref)))
  );
  ...
  server.tool(
    ...
    async ({ projectId, connectionId, search, ref, perPage }) =>
      toolResult(await gitlabSearchCode(connectionId ?? undefined, projectId, search, ref ?? undefined, perPage ?? undefined))
  );
}
