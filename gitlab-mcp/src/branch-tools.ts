...
export function registerBranchTools(server: McpServer): void {
  server.tool(
    ...
    async ({ projectId, connectionId, search, perPage }) =>
      toolResult(await gitlabListBranches(connectionId ?? undefined, projectId, search ?? undefined, perPage ?? undefined))
  );

  server.tool(
    "gitlab_get_branch",
    "【唯讀】取得單一分支的詳細資訊（最新 commit、是否受保護等）。",
    { projectId: projectIdParam, connectionId: connectionIdParam, branch: z.string().describe("分支名稱，區分大小寫") },
    async ({ projectId, connectionId, branch }) => toolResult(await gitlabGetBranch(connectionId ?? undefined, projectId, branch))
  );
}
