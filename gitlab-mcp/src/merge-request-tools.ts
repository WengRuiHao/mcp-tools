...
export function registerMergeRequestTools(server: McpServer): void {
  server.tool(
    ...
    async ({ projectId, state, targetBranch, sourceBranch, search, perPage, page }) =>
      toolResult(
        await gitlabListMergeRequests(projectId, {
  ...
  server.tool(
    ...
    { projectId: projectIdParam, mrIid: mrIidParam },
    async ({ projectId, mrIid }) => toolResult(await gitlabGetMergeRequest(projectId, mrIid))
  );
  ...
  server.tool(
    ...
    { projectId: projectIdParam, mrIid: mrIidParam },
    async ({ projectId, mrIid }) => toolResult(await gitlabGetMergeRequestChanges(projectId, mrIid))
  );
  ...
  server.tool(
    ...
    async ({ projectId, mrIid, perPage, page }) =>
      toolResult(await gitlabListMergeRequestDiscussions(projectId, mrIid, perPage ?? undefined, page ?? undefined))
  );
}
