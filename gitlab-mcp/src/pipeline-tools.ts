...
export function registerPipelineTools(server: McpServer): void {
  server.tool(
    ...
    async ({ projectId, ref, status, perPage, page }) =>
      toolResult(
        await gitlabListPipelines(projectId, {
          ref: ref ?? undefined,
  ...
  server.tool(
    ...
    { projectId: projectIdParam, pipelineId: pipelineIdParam },
    async ({ projectId, pipelineId }) => toolResult(await gitlabGetPipeline(projectId, pipelineId))
  );
  ...
  server.tool(
    ...
    async ({ projectId, pipelineId, perPage }) => toolResult(await gitlabListPipelineJobs(projectId, pipelineId, perPage ?? undefined))
  );
}
